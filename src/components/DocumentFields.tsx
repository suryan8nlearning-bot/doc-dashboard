...
// Add: helper to find mail instruction text in common locations
function getMailInstruction(doc: DocumentData): string {
  const candidates: Array<any> = [
    (doc as any)?.email_instruction,
    (doc as any)?.mail_instruction,
    (doc as any)?.document?.mail_instruction,
    (doc as any)?.document?.mail_instructions,
    (doc as any)?.document?.email_instruction,
    (doc as any)?.document?.pages?.[0]?.other?.mail_instruction,
    (doc as any)?.document?.pages?.[0]?.mail_instruction,
  ];
...
export function DocumentFields({ documentData, onFieldHover }: DocumentFieldsProps) {
  ...
  const mailInstruction = getMailInstruction(documentData);
  ...
  const SectionHeader = ({ title, id, color }: { title: string; id: string; color?: string }) => {
    ...
    return (
      <button
        ...
        className={`flex items-center justify-between w-full py-2.5 px-3 hover:bg-muted/50 transition-colors rounded-lg group ${isExpanded ? '' : 'bg-muted/40 border border-dashed border-muted/60'}`}
      >
        <div className="flex items-center gap-2">
          {color && <span className="inline-block h-3 w-1.5 rounded-sm" style={{ backgroundColor: color }} />}
          <h3 className={`font-semibold text-sm ${isExpanded ? 'text-foreground' : 'text-muted-foreground'} group-hover:text-primary transition-colors`}>
            {title}
          </h3>
        </div>
        {isExpanded ? (
  ...
  // Wrap content with a TooltipProvider for popups
  return (
            ...
            {expandedSections.has('mail_instruction') && (
              ...
              >
                <Textarea
                  ...
                  readOnly
                  className="min-h-[90px] bg-muted/40 text-muted-foreground"
                />
              </motion.div>
            )}
          </div>
